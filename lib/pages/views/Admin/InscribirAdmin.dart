...
class InscribirAdministrador extends StatefulWidget {

  @override
  _InscribirAdministradorState createState() => _InscribirAdministradorState();
}

class _InscribirAdministradorState extends State<InscribirAdministrador> {
                ...
                TextButton(onPressed: () async {
                  if(_formKey.currentState!.validate()){
                      ...
                      if(inscripcionConfirmada != null && inscripcionConfirmada){
                        crearAdmin(controladores,context);
                      }
                    
                  }
                },child: Text('Inscribir administrador',style:TextStyle(fontSize: 20,fontWeight:FontWeight.w600)))
                ]
                ),
              )
            )
        ...
        builder: (BuildContext context) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Center(
              child: ListBody(
                children: [
                  Center(child: Text('Administrador',style:TextStyle(fontWeight: FontWeight.bold))),
                  ...
                  Center(child:Column(
                    children: [
                      ...
                      Wrap(
                        children:[
                        Text('Estas seguro de querer inscribir este administrador? Recurde que tendra acceso completo al sistema!!',
  ...
  void crearAdmin(Map<String, dynamic> infoAdmin, BuildContext context) {
    final Usuarios adminNuevo = Usuarios.fromForm(formInfoIntoMap(infoAdmin));
    ScaffoldMessenger.of(context).showSnackBar(loadingSnackbar(
      message:'Registrando administrador...',
      onVisible: () async {        
        try {
          await controladorUsuario.registrar(adminNuevo);
          ScaffoldMessenger.of(context).removeCurrentSnackBar();
          ScaffoldMessenger.of(context).showSnackBar(successSnackbar('Administrador creado con exito!'));
          resetForm();
        } catch (e) {
          if(e.toString().contains('UNIQUE constraint failed')){
            ScaffoldMessenger.of(context).removeCurrentSnackBar();
            ScaffoldMessenger.of(context).showSnackBar(failedSnackbar('Ya existe un administrador con esa cedula'));
          }
          else {
            print(e);
            ScaffoldMessenger.of(context).removeCurrentSnackBar();
            ScaffoldMessenger.of(context).showSnackBar(failedSnackbar('Hubo un error al crear el administrador'));
          }
